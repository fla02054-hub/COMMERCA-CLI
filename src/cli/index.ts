...
import { discoverProductFromImage } from "../product/image-search.js";
...
if(args[0]==="account"){
  const action=args[1];
  if(action==="status"){console.log(JSON.stringify({accounts:listAccounts()},null,2));process.exit(0);}
  if(action==="login"){
    const target=args[2];if(!target)throw new Error(`usage: account login <service|all>`);
    if(target==="all"){for(const service of Object.keys(ACCOUNT_SERVICES) as AccountService[]) await loginAccount(service);} else await loginAccount(accountService(target));
    process.exit(0);
  }
  if(action==="open"){const target=args[2];if(!target)throw new Error(`usage: account open <service>`);await openAccount(accountService(target));process.exit(0);}
...
if(args[0]==="workflow"&&args[1]==="agent"){
  ...
  if(args.includes("--watch")){
    const intervalMs=Math.max(5000,Number(process.env.COMMERCA_AGENT_POLL_MS??"30000"));console.log(`COMMERCA autonomous agent worker started; polling every ${intervalMs}ms`);
    for(;;){for(const id of await listJobIds()){try{await runAgentJob(id,false);}catch(error){console.error(`Agent job ${id} failed:`,error instanceof Error?error.message:String(error));}}await new Promise(resolve=>setTimeout(resolve,intervalMs));}
  }
...
if(args[0]==="workflow"&&args[1]==="run"&&args.includes("--image")&&!args.includes("--product")){
  ...
  if(!existsSync(image))throw new Error(`Product image not found: ${image}`);
  console.log("COMMERCA-CLI\n\n[PRODUCT] Image-only mode started.");
  console.log("[PRODUCT] No product name, price, URL, or seller input is required.");
  const discovery=await discoverProductFromImage(image);
  console.log(`[PRODUCT] Search queries: ${discovery.queries.join(" | ")}`);
  console.log(`[PRODUCT] Candidates found: ${discovery.candidates.length}`);
  console.log(`[PRODUCT] Detailed matches: ${discovery.selected.length}`);
  if(!discovery.selected.length)throw new Error("No matching Shopee product could be verified from the image.");
  const best=discovery.selected[0];
  const product:Product={...best,id:`image-${crypto.randomUUID()}`,source:"shopee-image-discovery",discoveredAt:new Date().toISOString()};
