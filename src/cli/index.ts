...
if (args[0] === "workflow" && args[1] === "run" && args[2] === "--fixture" && args[3] === "ice-maker") {
  const product: Product = {
    id: "test-ice-maker-2026", name: "เครื่องทำน้ำแข็ง เครื่องทำน้ำแข็งอัตโนมัติ 2.3L ice maker เครื่องทำน้ำแข็งรุ่นใหม่ 2026",
    price: 2890, originalPrice: 3199, discount: Math.round(((3199 - 2890) / 3199) * 100), promotion: "ลดเหลือ ฿2,890 จากราคาปกติ ฿3,199",
    url: "https://s.shopee.co.th/6fgtBsfm7R", image: "fixture://ice-maker", images: ["fixture://ice-maker"], source: "manual-fixture", discoveredAt: new Date().toISOString(),
  };
...
if (args[0] === "workflow" && args[1] === "run" && args.includes("--product")) {
  process.env.COMMERCA_MODE = "live";
  ...
  if (!name || price === undefined || originalPrice === undefined || !url || !image) {
    throw new Error("usage: workflow run --product <name> --original-price <price> --price <special-price> --url <url> --image <image> [--images <image1> <image2> ...]");
  }
  if (originalPrice < price) throw new Error("Original price cannot be lower than the special price.");
  ...
  const promotion = originalPrice !== price ? `ลดเหลือ ฿${price.toLocaleString("th-TH")} จากราคาปกติ ฿${originalPrice.toLocaleString("th-TH")}` : undefined;
  const product: Product = { id: `manual-${crypto.randomUUID()}`, name, price, originalPrice, discount, promotion, url, image, images, source: "manual", discoveredAt: new Date().toISOString() };
