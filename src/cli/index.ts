...
async function runNewJob(){
 ...
 console.log(`COMMERCA-CLI\n\nJOB ID: ${jobId}\nSUB ID: ${subId}\nPRODUCT: ${name}\nPRICE: ฿${special.toLocaleString("th-TH")} (ปกติ ฿${original.toLocaleString("th-TH")})\nDISCOUNT: ${product.discount}%\nSTATUS: RUNNING`);
...
async function main(){if(args[0]!=="workflow")return usage();if(args[1]==="run")return runNewJob();if(args[1]==="approve")return approveJob();usage();}
