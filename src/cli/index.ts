...
function valueAfter(flag: string): string | undefined {
  ...
  const v = i < 0 ? undefined : args[i + 1];
...
function price(v: string | undefined, label: string): number | undefined {
...
function id(prefix: string): string {
  ...
  return `${prefix}-${timestamp}-${crypto.randomUUID().slice(0, 6).toUpperCase()}`;
}

function latestArtifact<T>(workflow: RuntimeWorkflow, type: string): T | undefined {
  return [...workflow.artifacts].reverse().find((a) => a.type === type)?.data as T | undefined;
}
...
async function runNewJob(): Promise<void> {
  ...
  if (!name || special === undefined || original === undefined || !url || !image) {
  ...
  const product: Product = {
    ...
    discount: original > 0 ? Math.round(((original - special) / original) * 100) : 0,
    promotion: original !== special ? `ลดเหลือ ฿${special.toLocaleString("th-TH")} จากราคาปกติ ฿${original.toLocaleString("th-TH")}` : undefined,
    url,
    image,
    images,
    subId,
