...
async function gemini(parts: AIPart[], context?: Record<string, unknown>, temperature = 0.35): Promise<string> {
  const apiKey = typeof context?.geminiApiKey === "string" ? context.geminiApiKey : process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error("No AI provider is available. Configure GROQ_API_KEY or GEMINI_API_KEY.");
  const model = typeof context?.geminiModel === "string" ? context.geminiModel : process.env.GEMINI_MODEL || DEFAULT_GEMINI_MODEL;
...
async function groq(parts: AIPart[], context?: Record<string, unknown>, temperature = 0.35): Promise<string> {
...
async function ai(parts: AIPart[], context?: Record<string, unknown>, temperature = 0.35): Promise<string> {
  if (typeof context?.groqApiKey === "string" || process.env.GROQ_API_KEY?.trim()) {
    try { return await groq(parts, context, temperature); }
    catch (error) {
      const message = error instanceof Error ? error.message : String(error); const match = message.match(/\((\d{3})\)/); const status = match ? Number(match[1]) : 0;
      if (!isRetryable(status)) throw error;
      if (typeof context?.onProgress === "function") (context.onProgress as (message: string) => void)(`[AI] Groq unavailable (${status}); falling back to Gemini once.`);
    }
  }
  return gemini(parts, context, temperature);
}
...
export class AIAgent {
  readonly name = "Aiden";
  readonly role = "Conversational autonomous AI agent operating COMMERCA with Groq as the primary model and Gemini as fallback";
