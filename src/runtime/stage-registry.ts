...
export function createStageRegistry(): WorkflowStageRegistry {
  ...
  registry.register(new FunctionStage("product-discovery", async (c) => {
    const results = await productRegistry().get("shopee-browser").search(c.goal);
  ...
  registry.register(new FunctionStage("product-research", async (c) => {
    const products = latest<unknown[]>(c, "product-candidate-list") ?? [];
  ...
  registry.register(new FunctionStage("product-analysis", async (c) => {
    const products = latest<Parameters<typeof rankProducts>[0]>(c, "product-profile") ?? [];
  ...
  registry.register(new FunctionStage("product-scoring", async (c) => {
    const analysis = latest<unknown[]>(c, "product-analysis") ?? [];
  ...
  registry.register(new FunctionStage("product-selection", async (c) => {
    const scored = latest<Array<{ product: unknown; score: number }>>(c, "scorecard") ?? [];
  ...
  registry.register(new FunctionStage("content-strategy", async (c) => {
    const selected = latest<{ product?: Parameters<typeof rankProducts>[0][number] }>(c, "selection");
  ...
  registry.register(new FunctionStage("decision-learning", async (c) => {
    ...
    return { artifacts: [artifact("decision-learning", "decision", result)], nextStage: "product-research" };
  }));
