...
import { ProductProviderRegistry, ShopeeBrowserProvider, RakatookyangProvider, rankProducts, readShopeeProductDetail, readRakatookyangPriceHistory } from "../product/index.js";
...
async function enrichProduct(product: Product): Promise<Product> {
  ...
  if (product.source === "shopee-browser" && /shopee\.co\.th/i.test(product.url)) return readShopeeProductDetail(product.url);
  if (product.source === "rakatookyang" && /shopee\.co\.th/i.test(product.url)) return readRakatookyangPriceHistory(product.url);
  return product;
}
...
export function createStageRegistry(options: StageRegistryOptions = {}): WorkflowStageRegistry {
  ...
  registry.register(new FunctionStage("product-discovery", async (c) => {
    const results = options.discoverProducts ? await options.discoverProducts(c.goal) : (await productRegistry().searchAll(c.goal)).flatMap((r) => r.products);
    const unique = [...new Map(results.map((p) => [p.url || `${p.source}:${p.name}`, p])).values()];
  ...
  registry.register(new FunctionStage("product-research", async (c) => {
    ...
    const researcher = options.researchProduct ?? enrichProduct;
    const researched: Product[] = [];
    const errors: Array<{ productId: string; error: string }> = [];
    for (const product of products) {
      try { researched.push(await researcher(product)); }
      catch (error) { errors.push({ productId: product.id, error: error instanceof Error ? error.message : String(error) }); researched.push(product); }
    }
    return { artifacts: [artifact("product-research", "product-profile", { products: researched, researchErrors: errors })] };
  }));
  ...
  registry.register(new FunctionStage("creative-strategy", async (c) => ({ artifacts: [artifact("creative-strategy", "creative-strategy", { image: ["product hero"], video: ["short product demonstration"], storyboard: ["hook", "problem", "product", "proof", "CTA"], prompt: [JSON.stringify({ content: latest(c, "content-package"), format: "9:16" })] } satisfies CreativeStrategy)] })));
  registry.register(new FunctionStage("production", async (c) => {
    ...
    return { artifacts: [artifact("production", "production-package", { image: creative.image, video: creative.video, editing: creative.storyboard } satisfies ProductionPackage)] };
  }));
  registry.register(new FunctionStage("qc", async (c) => {
    if (!latest<ProductionPackage>(c, "production-package")) throw new Error("QC requires production package.");
