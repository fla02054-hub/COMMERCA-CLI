...
function marketRegistry(): MarketResearchRegistry {
  ...
  registry.register(new FixtureMarketResearchProvider());
  return registry;
}
...
export function createStageRegistry(options: StageRegistryOptions = {}): WorkflowStageRegistry {
  ...
  registry.register(new FunctionStage("product-discovery", async (c) => {
    const results = options.discoverProducts
      ...
      : (await productRegistry().searchAll(c.goal)).flatMap((r) => r.products);
  ...
  registry.register(new FunctionStage("product-research", async (c) => {
    ...
    for (const product of products) {
      try {
        researched.push(await researcher(product));
      } catch (error) {
        errors.push({ productId: product.id, error: error instanceof Error ? error.message : String(error) });
        researched.push(product);
      }
    }
  ...
  registry.register(new FunctionStage("market-research", async (c) => {
    ...
    const evidence = await (options.marketResearch
      ? options.marketResearch({ productId: product.id, productName: product.name, query: c.goal })
      : marketRegistry().research({ productId: product.id, productName: product.name, query: c.goal }));
  ...
  registry.register(new FunctionStage("content-strategy", async (c) => {
    ...
    return {
      artifacts: [artifact("content-strategy", "content-package", generateContent(rankProducts([selected.product])[0]))],
    };
  }));

  registry.register(new FunctionStage("creative-strategy", async (c) => ({
    artifacts: [artifact("creative-strategy", "creative-strategy", {
      image: ["product hero"],
      video: ["short product demonstration"],
      storyboard: ["hook", "problem", "product", "proof", "CTA"],
      prompt: [JSON.stringify({ content: latest(c, "content-package"), format: "9:16" })],
    } satisfies CreativeStrategy)],
  })));
  ...
  registry.register(new FunctionStage("production", async (c) => {
    ...
    return { artifacts: [artifact("production", "production-package", {
