...
import { withAccountPage } from "../accounts/account-manager.js";
...
/** Keep Shopee short links intact so the connected Account Center profile performs the redirect. */
export async function resolveShopeeUrl(input: string): Promise<string> {
  return input.trim();
}

export async function readShopeeProductDetail(url: string, options: { waitMs?: number } = {}): Promise<ShopeeProductDetail> {
  return withAccountPage("shopee", async (page) => {
    await page.goto(await resolveShopeeUrl(url), { waitUntil: "domcontentloaded", timeout: 60000 }).catch(() => undefined);
    await page.waitForTimeout(options.waitMs ?? 12000);

    const extract = async () => page.evaluate(() => {
      const clean = (value: unknown) => String(value ?? "").replace(/\s+/g, " ").trim();
      const text = (node: Element | null) => clean(node?.textContent || node?.innerHTML || "");
      ...
      const product = jsonValues.find((value) => value && typeof value === "object" && value["@type"] === "Product") as Record<string, any> | undefined;
    ...
    if (data.blocked) throw new Error("Shopee verification is active in the connected Account Center session. Complete verification, then run the same command again.");
    if (!data.isProductPage || !data.name) { await page.waitForTimeout(5000); data = await extract(); }
    if (data.blocked) throw new Error("Shopee verification is active in the connected Account Center session. Complete verification, then run the same command again.");
    if (!data.name || !data.isProductPage) throw new Error("Shopee link did not resolve to a product page in the connected Account Center session.");
    ...
    return {
      ...
      source: "shopee-account-center", discoveredAt: new Date().toISOString(),
    };
  });
}
