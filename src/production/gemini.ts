...
export async function geminiImage(prompt: string): Promise<{ provider: "gemini"; model: string; mimeType: string; data: string }> {
  ...
  const result = await jsonRequest("/interactions", {
    model,
    input: prompt,
...
export async function geminiVoice(text: string): Promise<{ provider: "gemini"; model: string; mimeType: string; data: string }> {
  ...
  const result = await jsonRequest("/interactions", {
    model,
    input: `Synthesize this narration naturally in Thai. Do not add or remove words.\n\n${text}`,
...
export function buildSubtitle(storyboard: string[]): string {
  return storyboard.map((line, index) => `${index + 1}\n00:00:${String(index * 3).padStart(2, "0")},000 --> 00:00:${String(index * 3 + 3).padStart(2, "0")},000\n${line}\n`).join("\n");
}
