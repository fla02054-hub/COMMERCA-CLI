...
export function buildCreativeStrategy(content: ContentPackage): CreativeStrategy {
  ...
  const storyboard = [
    ...
    `Scene 4 — PROOF: Present only verified product evidence from the caption: ${content.caption}.`,
    `Scene 5 — DEAL: Make the price/promotion visually prominent. Keep every number exactly as provided.`,
    `Scene 6 — CTA: ${cta}. End with a clear visual instruction to tap the product link.`,
  ];
  ...
  return {
    ...
    storyboard,
    prompt: [
      `Create a high-converting vertical 9:16 ecommerce ad for ${content.title}. Use this exact hook: ${hook}. Show the real product clearly in every relevant shot. Do not invent claims, logos, prices, discounts, reviews, or features.`,
...
export function validateCreativeStrategy(strategy: CreativeStrategy): string[] {
  ...
  if (strategy.storyboard.length < 6) errors.push("storyboard must contain at least 6 sales beats");
