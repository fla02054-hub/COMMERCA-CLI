...
test("QC failure is repaired and the same job continues until QC passes",async()=>{ const registry=new WorkflowStageRegistry(); let qcAttempts=0; let productionAttempts=0; for(const stage of WORKFLOW_STAGES) registry.register(new FunctionStage(stage,async()=>{ if(stage==="production") productionAttempts++; if(stage==="qc"){ qcAttempts++; if(qcAttempts===1) return {artifacts:[{stage,type:"qc-report",data:{passed:false,issues:["invalid video fixture"],revisionStage:"production"},createdAt:new Date().toISOString()}]}; return {artifacts:[{stage,type:"qc-report",data:{passed:true,issues:[]},createdAt:new Date().toISOString()}]}; } return {artifacts:[{stage,type:`${stage}-ok`,data:{},createdAt:new Date().toISOString()}]}; })); const result=await executeWorkflow(createRuntimeWorkflow("qc repair test"),registry,{autonomous:true,maxAutonomousRevisions:1}); assert.equal(result.state.status,"completed"); assert.equal(result.state.currentStage,"final-package"); assert.equal(qcAttempts,2); assert.equal(productionAttempts,2); assert.equal(result.state.stages.find(s=>s.stage==="qc")?.status,"completed"); assert.equal(result.state.stages.find(s=>s.stage==="final-package")?.status,"completed"); assert.deepEqual(result.state.transitionHistory,["product-input","product-analysis","content-creative","production","qc","production","qc","final-package"]); assert.ok(result.artifacts.some(x=>x.type==="agent-decision")); });

test("successful workflow has an explicit terminal state",async()=>{ const registry=new WorkflowStageRegistry(); for(const stage of WORKFLOW_STAGES) registry.register(new FunctionStage(stage,async()=>({artifacts:[{stage,type:`${stage}-ok`,data:{},createdAt:new Date().toISOString()}]}))); const result=await executeWorkflow(createRuntimeWorkflow("terminal test"),registry); assert.equal(result.state.status,"completed"); assert.equal(result.state.currentStage,"final-package"); assert.deepEqual(result.state.transitionHistory,WORKFLOW_STAGES); assert.equal(result.state.transitionHistory.length,6); });
